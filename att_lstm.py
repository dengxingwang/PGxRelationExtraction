...
class AttLSTM:
    def __init__(self, config):
        ...
        with tf.variable_scope('dropout-embeddings'):
            self.embedded_chars = tf.nn.dropout(embedding_inputs, self.config.emb_dropout_keep_prob)

        # Bidirectional LSTM
        with tf.variable_scope("bi-lstm"):
            _fw_cell = tf.nn.rnn_cell.LSTMCell(self.config.hidden_size, initializer=initializer())
            fw_cell = tf.nn.rnn_cell.DropoutWrapper(_fw_cell, self.config.rnn_dropout_keep_prob)
            _bw_cell = tf.nn.rnn_cell.LSTMCell(self.config.hidden_size, initializer=initializer())
            bw_cell = tf.nn.rnn_cell.DropoutWrapper(_bw_cell, self.config.rnn_dropout_keep_prob)
        ...
        with tf.variable_scope('dropout'):
            self.h_drop = tf.nn.dropout(self.attn, self.config.dropout_keep_prob)

        # Fully connected layer
        # with tf.variable_scope('output'):
        #     self.logits = tf.layers.dense(self.h_drop, self.config.num_labels, kernel_initializer=initializer())
        ...
        #     self.loss = tf.reduce_mean(losses) + self.config.l2_reg_lambda * self.l2
        with tf.name_scope('logits'):
            self.logits = tf.layers.dense(self.h_drop, self.config.num_labels, name='logits')
