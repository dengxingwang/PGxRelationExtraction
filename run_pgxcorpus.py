...
class TextConfig():
    ...
    num_filters = 32  # number of convolution kernel
    filter_sizes = [2, 3, 4]  # size of convolution kernel
    hidden_dim = 32  # number of fully_connected layer units
    ...
    output_dir = './result_batch_size2/'
    data_dir = './corpus/PGXcorpus/'  # the path of input_data file
    training_data = data_dir + 'result_fold5_self_training.txt'
    dev_data = data_dir + 'result_fold5_self_test.txt'
    test_data = data_dir + 'result_fold5_self_test.txt'

    BERT_BASE_DIR = './pretrained_bert_model/'
    ...
    init_checkpoint = BERT_BASE_DIR + 'model.ckpt-1000000'  # the path of bert model
...
class Processor(object):
    ...
    def get_labels(self):
        """See base class."""
        return ['isAssociatedWith', 'influences', 'isEquivalentTo', 'decreases', 'treats', 'causes', 'increases']
...
def train():
    ...
    tensorboard_dir = os.path.join(config.output_dir, "tensorboard/pgxcorpus_self")
    save_dir = os.path.join(config.output_dir, "checkpoints/pgxcorpus_self")
